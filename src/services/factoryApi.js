...
function transformData(data) {
  ...
  const transformedDevices = devicesData.map(device => ({
    ...
    status: getDeviceStatus(device.Status || device.status, device.Connected || device.connected),
    connected: (device.Connected || device.connected) === "1" || (device.Connected || device.connected) === "Yes",
    warning: (device.Alarm || device.alarm) !== "0" && (device.Alarm || device.alarm) !== "",
