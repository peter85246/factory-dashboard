...
export const ProductionHistory = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (timeRange === 'year') {
      const monthlyData = Array.from({ length: 12 }, (_, i) => {
        ...
        const totalProduction = monthData.reduce((sum, item) => sum + item.production, 0);
  ...
  const productionStats = useMemo(() => {
    ...
    const maxProduction = Math.max(...filteredData.map(item => item.production));
    const minProduction = Math.min(...filteredData.map(item => item.production));
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={filteredData}>
              ...
              <Tooltip
                ...
                labelFormatter={(label) => `${timeRange === 'year' ? selectedYear + '年' : selectedYear + '年' + selectedMonth + '月'}${label}`}
              />
